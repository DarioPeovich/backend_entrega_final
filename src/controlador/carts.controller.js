...
import { sendPurchaseOk } from "../utils/email.js";
    ...
    try {
      ...
      if (cart) {
        ...
        for (let i = 0; i < cart.products.length; i++) {
          ...
          cartProduct.description = productDB.description; //Se le agrega la descripcion para usar en el email
        ...
        if (totalTicket == 0) {
          return res.send({
            status: "error",
            error:
              "Todas las quantity de los productos, están por encima de su respectivos Stock ",
          });
        }
        ...
        const resultCartAct = await CartsController.actualizarCart(
          cartId,
          ticketProducts,
          rejectedProducts
        ); //Se borra los productos que se incluyeron  en el Ticket
        await sendPurchaseOk(
          ticketCreated.purchaser,
          ticketCreated,
          ticketProducts
        ); //Se envia email de compra.
  ...
  static actualizarStock = async (ticketProducts) => {
    try {
      ticketProducts.forEach(async (product) => {
        // Utiliza async aquí para poder usar await dentro de la función de devolución de llamada
      ...
      console.error(
        `Error actualizando el stock para ${product.title}: ${error.message}`
      );
    }
  };
